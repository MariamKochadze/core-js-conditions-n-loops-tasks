...
function canQueenCaptureKing(queen, king) {
  ...
  const originMinX = minValue.x;
  const originMinY = minValue.y;

  for (let i = 0; i < 8; i += 1) {
    minValue.x += 1;
    minValue.y += 1;
    if (minValue.x === maxValue.x && minValue.y === maxValue.y) result = true;
  }

  minValue.x = originMinX;
  minValue.y = originMinY;
